...
def form_createAC():
    ...
    try:
        ...
        if flag == "true":
            cursor.execute("INSERT INTO player(name, username, pw, language, gender, email, status, createdSince) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",(name,uname,pw,language,gender,email,"verifying",dt))
            db.commit()
